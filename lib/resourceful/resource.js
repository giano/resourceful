...
function relationship(factory, type, r, options) {
  ...
  if (typeof(r) === 'string') {
	...
	if (type !== 'has_one') {
	  if (typeof rfactory === 'undefined') {
		...
		if (typeof rfactory === 'undefined') {
	    if (typeof resourceful.deferredHasOne[r] === 'undefined') {
	      resourceful.deferredHasOne[r] = [factory.resource]
	    } else {
	      resourceful.deferredHasOne[r].push(factory.resource)
	    }
	    return;
	  }  
	}
  ...
  rstringp = resourceful.pluralize(rstring);
	...
	if (type == 'has_one') {
    if (factory._has_one.indexOf(rstringc) !== -1) return;

    factory._has_one.push(rstringc);
    //
    ...
    factory.prototype['set' + rstringc] = function (value, callback) {
      if(value){
      	this[rstring + '_id']=value.id;
      }else{
      	this[rstring + '_id']=null;
      }
			this.save(callback);
    };
		
		
    rfactory.before('destroy', function(obj, next) {
      obj = obj[rfactory.key] || obj;
      obj = obj.split('/').slice(1).join('/');
			factory['by' + rstringc](obj, function(e, c) {
				if(e) {return next(e);}
				var new_val = {}
				new_val[rstring + '_id'] = null;
    ...
    factory['create' + rstringc] = function (parent, child, callback) {
      var key = factory.lowerResource + '_id',
          ...
          cid = child[rfactory.key] || resourceful.cuid();
...
Resource.has_one = function (resource, options) {
  relationship(this, 'has_one', resource, options);
};
...
Resource.date = function (name, schema) {
  return this.property(name, [Date,'any',null], schema);
};
...
Resource.method = function (name, fn, schema) {
  ...
  if (typeof schema === 'object') {
		var schema_clone = resourceful.clone(schema);
		schema_clone.properties = schema_clone.properties || {};
		if(schema_clone.properties.cb){
			schema_clone.properties.cb.type = ['any'];
		}else if(schema_clone.properties.callback){
			schema_clone.properties.callback.type = ['any'];
		}
    this[name] = function(){
      ...
      args.forEach(function(arg, i){
        ...
        //
        Object.keys(schema_clone.properties).forEach(function(prop, j){
          if(j === i) {
            payload[prop] = arg;
          }
        });
      ...
      //
      Object.keys(schema_clone.properties).forEach(function(prop, j){
        if(typeof payload[prop] === "undefined" && typeof schema_clone.properties[prop].default !== 'undefined') {
          payload[prop] = schema_clone.properties[prop].default;
        }
      });
			
			if(typeof args.cb === "function") {
				if(schema.properties.callback){
					payload.callback=args.cb;
				}else if(schema.properties.cb){
					payload.cb=args.cb;
				}
			}
      ...
      //
      var valid = validator.validate(payload, schema_clone);
      if(!valid.valid) {
        if(typeof args.cb === 'function') {
          args.cb(valid);
        } else {
...
Resource.prototype.toJSON = function () {
	...
	var out= resourceful.clone(this.properties);
	for(key in schema.properties){
		if(schema.properties[key].get){
			out[key]=this[key];
		}
	}
  return out;
...
Resource.prototype.safeJSON = function () {
  ...
  var schema = this.constructor.schema;
  return resourceful.clone(json, function (key) {
