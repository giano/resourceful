...
resourceful.define = function (name, definition) {
  ...
  if(typeof resourceful.deferredHasOne[Factory.resource] !== 'undefined') {
    //
    // For every deferredRelationship we find to our current resource, call it!
    //
    resourceful.deferredHasOne[Factory.resource].forEach(function(r){
      resourceful.resources[Factory.resource].has_one(r);
    });
  }
  return Factory;
};
