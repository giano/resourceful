		...
		map: function(doc) {
			...
			if (function() {
				...
				function get_value(object, key) {
				  if (key.indexOf(".") !== -1) {
				    return eval("(function(){ try{return object." + key + ";}catch(e){return null;}})()");
				  } else {
				    return object[key];
				  }
				};
				...
				function compare_values(value_in, expected_value, comparer) {
				  ...
				  switch (comparer) {
				    ...
				    case "is_not_in":
				      ...
				      return true;
				    case "contains":
				      if (typeof value_in !== "object" || !(value_in instanceof Array)) {
				        value_in = [value_in];
				      }
				      for (_k = 0, _len2 = value_in.length; _k < _len2; _k++) {
				        sub_value = value_in[_k];
				        if (compare_values(sub_value, expected_value, "==")) {
				          return true;
				        }
				      }
				      return false;
				...
				function compare(obj_value, expected_value) {
				  ...
				  if (typeof expected_value === "object") {
				    ...
				    if (expected_value.$notin != null) {
				      return compare_values(obj_value, expected_value.$notin, "is_not_in");
				    }
				    if (expected_value.$contains != null) {
				      return compare_values(obj_value, expected_value.$contains, "contains");
				    }
				    if (expected_value.$notcontains != null) {
				      return compare_values(obj_value, expected_value.$notcontains, "not_contains");
				    }
