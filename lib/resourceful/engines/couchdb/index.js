...
Couchdb.prototype.find = function (conditions, callback) {
  this.connection.temporaryView(resourceful.render({
    map: function (doc) {
      ...
      if (function () {
        for (var k in obj) {
					if ((typeof obj[k] == "object") && (obj[k] instanceof Array) && obj[k].length == 2){
						if doc[k] < obj[k][0] return false;
						if doc[k] > obj[k][1] return false;
					} else if (obj[k] instanceof RegExp){
						var val_of_el = (doc[k] || "").toString();
						if (!obj[k].test(val_of_el)) return false;
					} else if (obj[k] !== doc[k]){
						return false;
					}
        }
